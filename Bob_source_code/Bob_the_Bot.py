...
async def roll_call(ctx, start_time_utc=18, slot1react=':green_apple:',
    ...
    for i in range(12):
        ...
        if (today.strftime("%a") in days_list) and first_day_week:
            ...
            timemarker1 = calendar.timegm(
                today.replace(hour=start_time_utc, minute=0, second=0, microsecond=0).timetuple())
            timemarker2 = calendar.timegm(
                today.replace(hour=start_time_utc + 1, minute=0, second=0, microsecond=0).timetuple())
            timemarker3 = calendar.timegm(
                today.replace(hour=start_time_utc + 2, minute=0, second=0, microsecond=0).timetuple())
            timemarker4 = calendar.timegm(
                today.replace(hour=start_time_utc + 3, minute=0, second=0, microsecond=0).timetuple())
            timemarker5 = calendar.timegm(
...
async def helpmebob(ctx):
    ...
    await ctx.send(
        "Bob is a bot. And does Bob the Bot do?\n"
        "Bob the Bot does your beloved CB roll call.\n"
        "But how can I get Bob the Bot to do what Bob the Bot does?\n"
        "At the beginning of the CB week, type `/roll_call` followed by the UTC "
        "starting time of the CB session, and 5 emotes for people to react to.\n"
